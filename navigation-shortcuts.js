...
function hints(){document.querySelectorAll('[data-tool]').forEach((b,i)=>{const keys=Shortcuts.keys('developTab'+(i+1)).map(Shortcuts.label).join(' / ');b.title=b.dataset.tool+(keys?' ('+keys+')':'');});}
