...
Shortcuts.register('renameNode','Rename hovered or focused node','Editing',['r'],e=>{const focused=e.target.closest(selector),id=hovered||focused?.dataset.node||focused?.dataset.target,n=documentSettings.nodes.find(n=>n.uid===id);if(!img||!n)return;e.preventDefault();renameId=n.uid;$('renameNodeName').value=n.label;dialog.showModal();$('renameNodeName').focus();$('renameNodeName').select();});
