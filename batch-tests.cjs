...
(async()=>{
 ...
 catalog.projects[0].exportFolder=root;const bytes=new Uint8Array([1,2,3]);const first=await handlers['batch-export-write'](null,{token:session.token,id:'a',bytes}),second=await handlers['batch-export-write'](null,{token:session.token,id:'b',bytes});assert.equal(path.dirname(first),folder);assert.notEqual(first,second);assert.deepEqual(new Uint8Array(await fs.readFile(first)),bytes);
